...
export class RProxy {
    ...
    toJs(): ImplicitTypes {
        throw TypeError("JS conversion for this R object is not supported.");
    }
}
...
class RProxyInt extends RProxyVector {
    toJs(): number | ArrayBufferView{
        if (this.length == 1){
            ...
            let arrView = Module.HEAP32.subarray(
                Module._INTEGER(this.ptr)/4, Module._INTEGER(this.ptr)/ 4 + this.length
            )
            return arrView;
        }
    }
...
class RProxyReal extends RProxyVector {
    toJs(): number | ArrayBufferView{
        if (this.length == 1){
            ...
            let arrView = Module.HEAPF64.subarray(
                Module._REAL(this.ptr)/8, Module._REAL(this.ptr)/ 8 + this.length
            )
            return arrView;
        }
    }
...
class RProxyComplex extends RProxyVector {
    toJs(): Complex | Array<Complex>{
        if (this.length == 1){
            ...
            return [...Array(this.length)].map((_, idx) => {
                return {
...
class RProxyStr extends RProxyVector {
    toJs(): string | Array<string> {
        if (this.length == 1){
            return UTF8ToString(
                Module._R_CHAR(Module._STRING_ELT(this.ptr, 0))
            );
        } else {
            return [...Array(this.length)].map((_, idx) => {
                return UTF8ToString(
                    Module._R_CHAR(Module._STRING_ELT(this.ptr, idx))
                )
            });
        }
    }
...
class RProxyRaw extends RProxyVector {
    toJs(): string | Array<string> {
        let arrView = Module.HEAPU8.subarray(
            Module._RAW(this.ptr), Module._RAW(this.ptr) + this.length
        )
        return arrView;
    }
    get convertImplicitly(): boolean {
...
function getRProxyClass(type: SEXPTYPE) {
    const typeClasses: { [key in SEXPTYPE]? : any } = {
    ...
    throw TypeError(`RProxy SEXP type ${SEXPTYPE[type]} has not yet been implemented.`);
}

declare var Module: any;
const SEXPProxyHandlers = {
    get(target: RProxy, prop: string) {
        if (prop in target) {
