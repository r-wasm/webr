...
const server = createServer((req, res) => {
  try {
    ...
    if (url === '/' || url === '/index.html') {
      // Serve the test HTML from current directory
      filePath = join(__dirname, 'index.html');
    } else {
      // Serve all other files from ../../../../dist/
...
console.log('Serving webR files from ../../../../dist/');
...
(async () => {
  ...
  if (testStatus === 'success') {
    console.log('\n✓ Script module browser test passed!');
    process.exit(0);
  } else {
    console.error('\n✗ Script module browser test failed!');
    process.exit(1);
  }
})();
