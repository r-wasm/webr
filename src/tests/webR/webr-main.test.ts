...
describe('Test webR instance startup', () => {
  ...
  test('Can push shelter on the stack', async () => {
    ...
    expect(await webR.shelter.has(obj1)).toEqual(true);

    expect(await webR.shelter.push()).toEqual(2);
    expect(await webR.shelter.stackSize()).toEqual(2);

    const obj2 = await webR.evalR('2');
    expect(await webR.shelter.has(obj2)).toEqual(true);

    const obj3 = await webR.captureR('3');
    expect(await webR.shelter.has(obj3.result)).toEqual(true);
    ...
    expect(await webR.shelter.has(obj1)).toEqual(true);
    expect(await webR.shelter.has(obj2)).toEqual(false);
    expect(await webR.shelter.has(obj3.result)).toEqual(false);

    expect(await webR.shelter.pop()).toEqual(0);
    expect(await webR.shelter.stackSize()).toEqual(0);

    expect(await webR.shelter.has(obj1)).toEqual(false);
  });
});
...
describe('Evaluate R code', () => {
  ...
  test('Capture conditions while capturing R code', async () => {
    ...
    expect(await webR.shelter.has(cond[0].data)).toEqual(true);
...
describe('Create R objects using serialised form', () => {
  ...
  test('Create an atomic vector', async () => {
    ...
    expect(await webR.shelter.has(rObj)).toEqual(true);
