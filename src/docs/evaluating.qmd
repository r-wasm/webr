...
During capture with `evalR()` and `captureR()` the R session is set as non-interactive. After the output capture has completed, the session's interactive status will be restored.
