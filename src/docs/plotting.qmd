...
When the `webr::canvas()` graphics device is used it is the web browser that handles the specifics of text rendering, and so any fonts installed on the host machine can be used with the `family` argument when plotting. Modern features provided by the browser such as RTL text, ligatures, colour emoji, or the use of Arabic, Japanese or Cyrillic script should also be handled automatically.
...
<script type="module">
  ...
  import { WebR } from 'https://webr.r-wasm.org/main/webr.mjs';
  const webR = new WebR();
  await webR.init();
