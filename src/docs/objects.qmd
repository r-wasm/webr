...
To avoid increasingly nested blocks, it is often preferable to work with a shelter instead. A new shelter can be created using the [`Shelter`](api/js/classes/WebR.Shelter.md) class. An asynchronous `Shelter` constructor is available as an instance property of the `WebR` class. Creating a new object with this constructor returns a Promise that resolves to a new instance of the `Shelter` class once the shelter is ready to use.

- The objects referenced in a shelter can be destroyed all at once using the `purge()` method.
...
try {
  const obj1 = await myShelterR.evalR('42 + 123');
  const obj2 = await webR.evalR('42 * 123');

  const result = obj1.toNumber() + obj2.toNumber();
  console.log(result);
} finally {
  myShelterR.purge();
}
