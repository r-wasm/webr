...
WebR's support package includes a built in graphics device, `webr::canvas()`, designed for drawing graphics using the JavaScript [Canvas API](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API). This graphics device is implemented by rendering with an [`OffscreenCanvas`](https://developer.mozilla.org/en-US/docs/Web/API/OffscreenCanvas) object on the worker thread, and then transmitting the resulting image data back to the main thread as an [`ImageBitmap`](https://developer.mozilla.org/en-US/docs/Web/API/ImageBitmap).

Output messages containing canvas image data are of the form,

``` javascript
{ type: 'canvasImage', data: { image: ImageBitmap } }
```

The resulting image can be redrawn to any 2D canvas context that is available on the main thread. For example, for a fixed HTML canvas element with `id="plot-canvas"`, messages of type `canvasImage` could be handled by executing,

``` javascript
const canvas = document.getElementById('plot-canvas');
canvas.getContext('2d').drawImage(message.data.image, 0, 0);
```
