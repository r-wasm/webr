...
In addition to explicitly constructing an R object with `await new`, new R object construction is also performed automatically when [executing R functions with JavaScript arguments](convert-r-to-js.qmd#exec-r).

Newly created `RObject` references are protected by the [default shelter](objects.qmd#the-default-shelter) unless [otherwise specified](objects.html#working-with-multiple-shelters).
...
An [`RObject`](api/js/classes/RWorker.RObject.md) can be used as part of R object construction, either on its own, included in a JavaScript array, or as the values in an [`WebRDataTree`](api/js/modules/Tree.md#webrdatatree).
...
This mechanism can be useful for creating new R lists or environments, or when re-constructing partially serialised objects in the form of an [`WebRDataTree`](api/js/modules/Tree.md#webrdatatree).
