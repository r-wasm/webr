...
The public CDN build of webR has been distributed with a pre-installed [webr support package](rapi). This R package provides a helper function [`install()`](api/r.qmd#install-one-or-more-packages-from-a-webr-binary-package-repo), which can be used to install further R packages from a compatible webR binary repository.
