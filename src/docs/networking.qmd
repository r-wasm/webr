...
The main drawback of this approach is that it requires some setup outside of webR, which might not always be possible. Specifically, it requires the use of a WebSocket-to-TCP proxy to make network connections, and a SOCKS proxy for forwarding network requests.
